...
class CrossValidator:
    ...
    def generate_splits(self, df: pd.DataFrame) -> None:
        ...
        try:
            ...
            logger.info(f"Total trading days: {total_days}. Calculated test size per fold: {test_size} days.")
            ...
            logger.info(f"Successfully generated {len(self.date_folds)} folds. First fold train size: {len(self.date_folds[0][0])} days (> 2 years).")
    ...
    def plot_cv(self, save_dir: str = "results/cross-validation") -> None:
        ...
        try:
            ...
            for i, (train_dates, val_dates) in enumerate(self.date_folds):
                ...
                ax.scatter(train_idx, [i + 1] * len(train_idx), c="blue", marker="_", lw=8)
                ax.scatter(val_idx, [i + 1] * len(val_idx), c="orange", marker="_", lw=8)
