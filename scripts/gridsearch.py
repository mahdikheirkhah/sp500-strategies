...
class ModelTrainer:
    ...
    def __init__(
        self, df: pd.DataFrame, cv_folds: list[tuple[np.ndarray, np.ndarray]]
    ) -> None:
        ...
        logger.info(
            "Shifting target labels by +1 to satisfy XGBoost strict indexing (0, 1, 2)..."
        )
    ...
    def evaluate_and_extract(
        self, best_pipeline: Pipeline, save_dir: str = "results/cross-validation"
    ) -> None:
        ...
        try:
            for fold, (train_idx, val_idx) in enumerate(self.cv_indices):
                X_train, y_train = (
                    self.df.loc[train_idx, self.features],
                    self.df.loc[train_idx, self.target],
                )
                X_val, y_val = (
                    self.df.loc[val_idx, self.features],
                    self.df.loc[val_idx, self.target],
                )
                ...
                feat_df = pd.DataFrame(
                    {"Feature": surviving_features, "Importance": importances}
                )
            ...
            metrics_df.to_csv(
                os.path.join(save_dir, "ml_metrics_train.csv"), index=False
            )

            fi_df = pd.concat(feature_importance_list, ignore_index=True)
            fi_df.to_csv(
                os.path.join(save_dir, "top_10_feature_importance.csv"), index=False
            )

            plt.figure(figsize=(10, 6))
            plt.plot(
                metrics_df["Fold"],
                metrics_df["Train_AUC"],
                label="Train AUC",
                marker="o",
            )
            plt.plot(
    ...
    def save_model(
        self, grid_search: GridSearchCV, save_dir: str = "results/selected-model"
    ) -> None:
        ...
        try:
            ...
            joblib.dump(
                ...
                os.path.join(save_dir, "selected_model.pkl"),
            )
...
if __name__ == "__main__":
    try:
        ...
        for name, config in candidate_models.items():
            ...
            grid_result = trainer.execute_grid_search(
                pipeline, param_grid=config["params"]
            )
        ...
        logger.success(
            f"🏆 ULTIMATE WINNER: {best_overall_model.best_estimator_.named_steps['model'].__class__.__name__} with AUC: {best_overall_score:.4f} 🏆"
        )
