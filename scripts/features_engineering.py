...
class FeatureEngineer:
    ...
    def load_data(self) -> None:
        ...
        try:
            ...
            self.constituents_df["date"] = pd.to_datetime(
                self.constituents_df["date"], format="%Y-%m-%d"
            )
            self.constituents_df = self.constituents_df.sort_values(
                by=["Name", "date"]
            ).reset_index(drop=True)
            logger.info(
                f"Constituents data loaded successfully. Shape: {self.constituents_df.shape}"
            )
            ...
            if (
                "close" in self.index_df.columns
                and self.index_df["close"].dtype == "object"
            ):
                self.index_df["close"] = (
                    self.index_df["close"].astype(str).str.replace("$", "", regex=False)
                )
                self.index_df["close"] = (
                    self.index_df["close"]
            ...
            self.index_df["Date"] = pd.to_datetime(
                self.index_df["Date"], format="%m/%d/%y"
            )
            self.index_df = self.index_df.sort_values(by=["Date"]).reset_index(
                drop=True
            )
            logger.info(
                f"S&P 500 Index benchmark data loaded successfully. Shape: {self.index_df.shape}"
            )

        except FileNotFoundError as e:
            logger.error(
                f"Failed to locate data files. Ensure they are in the correct directory: {e}"
            )
            raise
    ...
    def define_target(self) -> None:
        ...
        try:
            ...
            self.constituents_df["target"] = np.sign(
                self.constituents_df["target_return"]
            )

            initial_len = len(self.constituents_df)
            self.constituents_df = self.constituents_df.dropna(
                subset=["target"]
            ).reset_index(drop=True)

            logger.info(
                f"Target defined successfully. Dropped {initial_len - len(self.constituents_df)} boundary rows with NaN targets."
            )
    ...
    def apply_technical_indicators(self) -> None:
        ...
        try:
            indicator_engine = TechnicalIndicators(
                group_column="Name", price_column="close"
            )
            self.constituents_df = indicator_engine.apply_indicators(
                self.constituents_df
            )
            logger.info(
                f"Indicators successfully integrated. New DataFrame shape: {self.constituents_df.shape}"
            )
        except Exception as e:
    ...
    def split_train_test(self) -> tuple[pd.DataFrame, pd.DataFrame]:
        ...
        try:
            train_df = self.constituents_df[
                self.constituents_df["date"].dt.year < 2017
            ].copy()
            test_df = self.constituents_df[
                self.constituents_df["date"].dt.year >= 2017
            ].copy()

            assert (
                train_df["date"].max() < test_df["date"].min()
            ), "CRITICAL: Train and Test sets overlap chronologically!"

            logger.info(
                f"Split complete. Train set size: {len(train_df)}, Test set size: {len(test_df)}."
            )
    ...
    def log_column_types(self) -> None:
        ...
        try:
            ...
            types_str = "\n".join(
                [f"  - {col}: {dtype}" for col, dtype in types_series.items()]
            )
...
if __name__ == "__main__":
    try:
        ...
        logger.success(
            "Feature Engineering Phase (Data Prep, Target, Indicators) completed successfully."
        )
