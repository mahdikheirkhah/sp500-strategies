...
class StrategyBacktester:
    ...
    """
    def __init__(self, model_path: str, signal_path: str, save_dir: str = "results/backtest") -> None:
    ...
    def prepare_full_timeline(self, engineer: FeatureEngineer) -> pd.DataFrame:
        ...
        try:
            ...
            test_probs = pipeline.predict_proba(test_df[self.features])[:, class_up_index]
            ...
            train_signal = pd.merge(train_signal, train_df[['date', 'Name', 'target_return']], on=['date', 'Name'], how='left')
            ...
            full_signal = full_signal.sort_values(by=['date', 'Name']).dropna(subset=['target_return'])
            
            logger.success(f"Timeline prepared. Total trading days evaluated: {full_signal['date'].nunique()}")
            return full_signal
    ...
    def execute_strategy(self, signal_df: pd.DataFrame) -> pd.DataFrame:
        ...
        """
        logger.info(f"Executing Long/Short Strategy (Long > {self.long_threshold}, Short < {self.short_threshold})...")
        try:
            # 1. Define Positions
            signal_df['position'] = 0
            signal_df.loc[signal_df['signal_prob'] > self.long_threshold, 'position'] = 1
            signal_df.loc[signal_df['signal_prob'] < self.short_threshold, 'position'] = -1
            
            # 2. Calculate individual raw trade return
            signal_df['trade_return'] = signal_df['position'] * signal_df['target_return']
            ...
            logger.warning(f"Risk Management: Stop-Loss (-5%) triggered on {num_stops_hit} individual trades.")
            ...
            daily_portfolio = active_trades.groupby(['date', 'dataset'])['trade_return'].mean().reset_index()
            daily_portfolio.rename(columns={'trade_return': 'strategy_return'}, inplace=True)
    ...
    def process_benchmark(self, engineer: FeatureEngineer, strategy_dates: pd.Series) -> pd.DataFrame:
        """
        ...
        try:
            ...
            benchmark['benchmark_equity'] = (1 + benchmark['benchmark_return']).cumprod()
    ...
    def plot_and_evaluate(self, portfolio: pd.DataFrame, benchmark: pd.DataFrame) -> None:
        """
        ...
        try:
            ...
            plt.plot(merged['date'], merged['strategy_cum_pnl'], label=f'ML Strategy', color='blue', lw=2)
            plt.plot(merged['date'], merged['benchmark_cum_pnl'], label='S&P 500 Benchmark', color='gray', alpha=0.6, lw=2)
            
            # Vertical Train/Test Split Line
            if pd.notnull(test_start_date):
                plt.axvline(x=test_start_date, color='red', linestyle='--', lw=2, label='Train / Test Split')
                plt.text(test_start_date, plt.ylim()[1] * 0.9, '  Test Set (Unseen Future) ->', color='red')

            plt.title("Algorithmic Strategy vs. S&P 500 Benchmark (Normalized $1/Day, Compounding)")
            plt.xlabel("Date")
